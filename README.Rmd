...
Sometimes it may be necessary to convert gene names between different conventions and possibly organisms. The rRoma package provides the fucntion `ConvertNames` that can be used to conver the genes specified by the Module list. The fucntion relies on the `biomaRt` package and therefore a working internet connection is required.

For example, to convert the MyoGS module list into the Ensembl format it is sufficient to write:
...
```{r}
MyoGS.ENS <- ConvertModuleNames(ModuleList = MyoGS, SourceOrganism = "hsapiens", TargetOrganism = "mmusculus",
                   SourceTypes = "Names", TargetTypes = "Names",
                   HOST = "grch37.ensembl.org", PATH = "/biomart/martservice")
```
